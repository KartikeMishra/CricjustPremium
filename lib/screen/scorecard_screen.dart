...
class _ScorecardScreenState extends State<ScorecardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          const MeshBackdrop(), // 🎨 soft background
          SafeArea(
  ...
  Widget _buildTabSelector(String teamA, String teamB, bool isDark) {
    return Padding(
      ...
      child: GlassPanel(
        ...
        darkColor: const Color(0xFF16181C),
        child: Stack(
          children: [
            ...
            const Positioned(
              right: -6,
              bottom: -8,
              child: WatermarkIcon(icon: Icons.sports_cricket, size: 92, opacity: 0.06),
            ),
            TabBar(
              indicator: BoxDecoration(
                color: isDark ? Colors.white24 : const Color(0xFF1976D2),
                borderRadius: BorderRadius.circular(30),
                boxShadow: isDark ? null : const [BoxShadow(color: Color(0x22000000), blurRadius: 8, offset: Offset(0, 3))],
              ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildCard(child: _buildBattingTable(bat), isDark: isDark, markIcon: Icons.sports_cricket),
          const SizedBox(height: 12),
          if (bat.details.yetToBat.isNotEmpty)
            _buildCard(
              isDark: isDark,
              markIcon: Icons.group,
              child: _yetToBatInner(bat, isDark),
            ),
          const SizedBox(height: 12),
          _buildCard(child: _buildBowlingTable(bowl), isDark: isDark, markIcon: Icons.sports_baseball),
  ...
  Widget _buildCard({
    ...
    IconData? markIcon, // 🎨 faint watermark icon (optional)
  }) {
    return GlassPanel(
        ...
        children: [
          ...
          if (markIcon != null)
            Positioned(
              right: -6,
              bottom: -6,
              child: WatermarkIcon(icon: markIcon, size: 96, opacity: 0.06),
            ),
          child,
        ],
      ),
