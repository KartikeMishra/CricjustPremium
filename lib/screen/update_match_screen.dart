...
class _UpdateMatchScreenState extends State<UpdateMatchScreen> {
  ...
  Future<void> _fetchMatchDetails() async {
    ...
    if (body['status'] == 1) {
      // The endpoint always returns an array of length 1:
      final rawList = body['data'] as List<dynamic>;
      if (rawList.isEmpty) return;

      // Safely cast the first element to Map:
      final data = rawList[0] as Map<String, dynamic>;

      setState(() {
        // Basic metadata
        _matchName = data['match_name'] as String? ?? '';
        _matchType = (data['tournament_id'] as int? ?? 0) > 0
            ? 'tournament'
            : 'one_to_one';
        ...
        if (venueId != null) {
          // later you can fetch details or match it from your _venues list
        ...
        // Note: API field is "ballers_max_overs"
      ...
      // Finally load the actual player objects for each ID (so you can show names/chips):
      if (_selectedTeamAPlayers.isNotEmpty) {
        await _fetchPlayers(teamId: _selectedTeamAId!, isTeamA: true);
      }
      if (_selectedTeamBPlayers.isNotEmpty) {
